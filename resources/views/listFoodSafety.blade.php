         ...
         <div class="card card_c mb-3">
            ...
            <div class="card-body">
               ...
               <div class="d-flex justify-content-between">
                  <a name="" id="" class="text_p font-weight-bold" href="
                     @if ($element->fds_instagram != null)
                        {{$element->fds_instagram}}
                     @elseif ($element->fds_facebook != null)
                        {{$element->fds_facebook}}
                     @elseif ($element->fds_youtube != null)
                        {{$element->fds_youtube}}
                     @elseif ($element->fds_url != null)
                        {{$element->fds_url}}
                     @endif
                     " role="link" target="_blank">{{$element->fds_source}}</a>
            ...
            <div class="d-flex justify-content-end p-1 bg_p">
               ...
               <!--<form action="{{route('alimentos.destroy', $element->id)}}" method="POST">
                     @method('delete')
                     @csrf
                     <button type="submit" class="btn btn_circl_outl_p m-1"><i class="fas fa-times"></i></button>
                  </form>-->
            </div>
            @endauth
         </div>
